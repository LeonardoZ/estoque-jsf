...
public class Pagamento extends Entidade {
	...
	return parcelas.stream().map(Parcela::valorASerPago).reduce(Dinheiro::somar).get();
    }
	...
	this.parcelas = parcelamento.calcularParcelas(valorLiquido(), new Date());
	this.parcelas.forEach(p -> p.setPagamento(this));
    }
